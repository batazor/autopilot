...
/**
 * RequestInit for a JSON-body mutation (POST/PUT/PATCH). Sets the
 * Content-Type header and serializes `body`; pass no body for an empty
 * request. Replaces the repeated `{ method, headers, body: JSON.stringify }`
 * literal sprinkled across the call sites below.
 */
...
export type DailyTasksView = {
  player_id: string;
  read: boolean; // false until the daily list has been OCR'd
  refresh_in_s: number | null; // seconds to the daily reset
  summary: { total: number; done: number; claimable: number; open: number };
  tasks: DailyTaskView[];
};

/** A player's parsed daily missions + completion status. */
export function fetchDailyTasks(playerId: string): Promise<DailyTasksView> {
  return apiFetch<DailyTasksView>(
    `/api/quests/daily/${encodeURIComponent(playerId)}`,
  );
}
...
export async function postAbortTask(
  ...
  await apiFetch<{ ok: boolean }>(
    ...
    jsonInit("POST", { restart: Boolean(opts?.restart) }),
  );
}
...
export async function runQueueTaskNow(taskId: string): Promise<boolean> {
  const data = await apiFetch<{ ok: boolean }>("/api/queue/run-now", jsonInit("POST", { task_id: taskId }));
  return data.ok;
}

export async function removeQueueTasks(taskIds: string[]): Promise<number> {
  const data = await apiFetch<{ removed: number }>("/api/queue/remove", jsonInit("POST", { task_ids: taskIds }));
  return data.removed;
}
...
export async function rescheduleQueueTask(
  ...
  const data = await apiFetch<{ ok: boolean }>("/api/queue/reschedule", jsonInit("POST", { task_id: taskId, scheduled_at: scheduledAt }));
  return data.ok;
}
...
export async function submitDecision(
  ...
  const data = await apiFetch<{ ok: boolean }>(
    ...
    jsonInit("POST", { decision, request_id: requestId }),
  );
  return data.ok;
}
...
export async function parseDreamscapeNames(
  ...
  return apiFetch<DreamscapeParseNamesResult>("/api/dreamscape/parse-names", jsonInit("POST", { text }));
}
...
export async function detectDreamscapeScene(
  ...
  return apiFetch<DreamscapeDetectSceneResult>("/api/dreamscape/detect-scene", jsonInit("POST", { words }));
}
...
export async function captureLabelingScreenshot(
  ...
  return apiFetch(`/api/labeling/capture${labelingScopeQuery(scope)}`, jsonInit("POST", { instance_id: instanceId }));
}
...
export async function refreshLabelingReference(
  ...
  return apiFetch(`/api/labeling/refresh${labelingScopeQuery(scope)}`, jsonInit("POST", { ref: refRel, instance_id: instanceId }));
}
...
export async function promoteLabelingReference(
  ...
  return apiFetch(`/api/labeling/promote${labelingScopeQuery(scope)}`, jsonInit("POST", {
    ref: refRel,
    basename,
...
export async function updateGiftCodeDiscordConfig(
  ...
  return apiFetch<GiftCodeDiscordConfig>("/api/gift-codes/discord-config", jsonInit("PUT", body));
}
...
export async function fetchWikiEntries(
  entity: "buildings" | "heroes" | "items" | "pets" | "experts",
  scope = "all",
  q = "",
...
export async function setScenarioEnabled(
  ...
  await apiFetch(`/api/modules/scenarios/${encodeURIComponent(key)}/enabled`, jsonInit("PATCH", { enabled }));
}
...
export async function createAdbDevice(body: DeviceCreateBody): Promise<DeviceRegisterResult> {
  return apiFetch<DeviceRegisterResult>("/api/adb/devices", jsonInit("POST", body));
}
...
export async function solveOptimizer(body: {
  ...
  return apiFetch<OptimizerSolveResult>("/api/optimizer/solve", jsonInit("POST", body));
}
...
export async function optimizerDryRun(body: {
  ...
  return apiFetch("/api/optimizer/dry-run", jsonInit("POST", body));
}
...
export async function optimizerApprove(body: {
  ...
  return apiFetch("/api/optimizer/approve", jsonInit("POST", body));
}
...
export async function optimizerQueue(body: {
  ...
  return apiFetch("/api/optimizer/queue", jsonInit("POST", body));
}
...
export async function saveEditScenarioFile(rel: string, yaml: string): Promise<void> {
  await apiFetch(`/api/edit-dsl/file?${editDslQuery({ rel })}`, jsonInit("PUT", { yaml }));
}
...
export async function saveEditScenarioDocument(
  ...
  await apiFetch(`/api/edit-dsl/file?${editDslQuery({ rel })}`, jsonInit("PUT", { document }));
}

export async function validateEditScenarioYaml(
  yaml: string,
): Promise<{ valid: boolean; error: string; preview: string }> {
  return apiFetch("/api/edit-dsl/validate", jsonInit("POST", { yaml }));
}

export async function validateEditScenarioDocument(
  document: EditScenarioDocument,
): Promise<{ valid: boolean; error: string; preview: string }> {
  return apiFetch("/api/edit-dsl/validate", jsonInit("POST", { document }));
}
...
export async function createEditDslFile(body: {
  ...
  return apiFetch("/api/edit-dsl/create", jsonInit("POST", body));
}

// --------------------------------------------------------------------------- //
// Planner calculators (what-if endpoints over the per-domain planners)
// --------------------------------------------------------------------------- //
...
/** Domain id → planner endpoint under /api/planner/<domain>. */
...
export async function fetchPlannerMeta(): Promise<PlannerMeta> {
  return apiFetch<PlannerMeta>("/api/planner/meta");
}
...
export type FullPlanResult = {
  plans: Record<string, PlannerResult>;
  candidates: PlannerResult[];
  decision: PlannerResult;
};

/** Run every domain planner and arbitrate via the coordinator (the unified plan). */
export async function computeFullPlan(body: unknown): Promise<FullPlanResult> {
  return apiFetch<FullPlanResult>("/api/planner/full", jsonInit("POST", body));
}

export type FleetPlanRow = {
  player_id: string;
  nickname: string;
  picks: { building?: string; research?: string; heroes?: string; pets?: string };
  committed: number;
  starved: string[];
  bottleneck: string[];
  error: string | null;
...
/** Run the full plan for every player at once (the fleet overview). */
export async function fetchFleetPlan(): Promise<FleetPlanRow[]> {
  const d = await apiFetch<{ rows: FleetPlanRow[] }>("/api/planner/fleet");
  return d.rows;
}

export async function fetchBuildingCatalog(): Promise<BuildingCatalogEntry[]> {
  const d = await apiFetch<{ buildings: BuildingCatalogEntry[] }>(
    "/api/planner/building/catalog",
  );
  return d.buildings;
}

/** Backfill prerequisite building levels implied by the given levels. */
export async function autofillBuildingLevels(
  levels: Record<string, unknown>,
): Promise<{ levels: Record<string, string>; added: string[] }> {
  return apiFetch("/api/planner/building/autofill", jsonInit("POST", { levels }));
}

export type PlayerPlannerState = {
  player_id: string;
  nickname: string;
  /** Per-domain planner input bodies, pre-filled from the player's saved state. */
  domains: Record<string, Record<string, unknown>>;
};

/** Load a player's saved planner inputs (manual layer + reader-owned overlays). */
export async function fetchPlayerPlannerState(
  playerId: string,
): Promise<PlayerPlannerState> {
  return apiFetch<PlayerPlannerState>(
    `/api/planner/state/${encodeURIComponent(playerId)}`,
  );
}

/** Persist the per-domain planner input bodies back to the player's state. */
export async function savePlayerPlannerState(
  playerId: string,
  domains: Record<string, Record<string, unknown>>,
): Promise<{ ok: boolean; saved_domains: string[] }> {
  return apiFetch(
    `/api/planner/state/${encodeURIComponent(playerId)}`,
    jsonInit("PUT", { domains }),
  );
}
