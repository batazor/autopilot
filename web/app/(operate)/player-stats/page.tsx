...
function PlayerStatsContent() {
  ...
  return (
    <div className="page-stack">
      <PageHeader title="Player statistics" fleet={{ showPlayer: true }}>
        <p>
      ...
      {!loading && stats ? (
        <>
          <MetricGrid className="mb-4">
            <MetricCard label="Nickname" value={stats.nickname || "—"} />
            <MetricCard
              label="Latest power"
            ...
            <MetricCard
              label="Furnace level"
              value={latest ? String(latest.furnace_level) : "—"}
            />
            <MetricCard label="Days tracked" value={stats.series.length} />
            <MetricCard label="Level-ups" value={stats.level_events.length} />
          </MetricGrid>
