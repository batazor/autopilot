...
export function OnboardingWizard() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const [envHealth, st, bot, adb] = await Promise.all([
          fetchEnvHealth(),
          fetchOnboardingState(),
          fetchBotStatus(),
          readAdbStatus(),
        ...
        if (envHealth.redis.ok && deviceReady && running) {
        ...
        if (!envHealth.redis.ok) setStep(STEP_ENVIRONMENT);
