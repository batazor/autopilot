"""Structural checks for ``modules/mail/scenarios/read_mail_gifts.yaml``.
...
SCENARIO_PATH = MODULE_DIR / "scenarios" / "read_mail_gifts.yaml"
...
def _assert_claim_block_shape(block: list, *, context: str) -> None:
    ...
    claim_guard = block[1]
    assert claim_guard.get("while_match") == "mail.claim_all", f"{context}: step 1 must guard on mail.claim_all"
    ...
    assert delete_guard.get("while_match") == "mail.delete_all", f"{context}: step 2 must guard on mail.delete_all"
