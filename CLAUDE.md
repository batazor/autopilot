...
references/           # Template crops (generated by Labeling, used by overlay engine)
db/                   # YAML data (devices.yaml, buildings/*.yaml, etc.)
area.json             # UI region/screen definitions (generated by Labeling editor)
```
...
3. Save → updates `area.json` and `references/` crops
...
```yaml
devices:
  - bluestacks_window_title: "instance-1"
...
4. Save → updates `area.json` and `references/crop/*.png`
...
3. Verify region exists in `area.json` and has a crop in `references/`
...
- **`area.json`** — generated UI region/screen definitions (do not edit manually)
