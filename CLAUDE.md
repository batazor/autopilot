...
- **To *show* the user a screenshot/image, render it with the `show_widget` (visualize) preview — `Read` does not display it.** Opening an image with `Read` (e.g. a `botctl screenshot bs1` PNG, a crop, an annotated frame) lets *Claude* see it but the user sees nothing. When the user needs to see a device screenshot or visual markup, render it inline via the visualize `show_widget` tool.
