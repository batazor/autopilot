...
A module's `__init__.py` exports:
- **`MODULE_ID`** — unique identifier
- **`MODULE_CONFIG`** — user settings (enable/disable, thresholds, etc.)
- **`area_yml()`** — screen/region definitions (flows into `area.json` merge)
- **`overlay_analyze_yaml()`** — overlay engine rules (detect UI state)
- **`scenarios()`** — list of DSL scenario files to load
...
**Do not edit `area.json` manually.** It is generated by the labeling UIs.

**Next.js** (`/labeling`, Konva): capture, regions, versions, basename promote/rename, crops — primary labeling UI (`uv run api` + `web` dev server).
