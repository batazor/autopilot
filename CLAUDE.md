...
- **To just *show* the user a device screen: run `uv run botctl screenshot <inst>` and STOP.** The PNG lands in the live preview the user already watches — that IS how they see it. Do NOT render anything. (User rule, verbatim: "не надо рендерить ничего просто сделал скриншот показал в превью … не понтуйся, тратишь наше время на фигню" — building a base64 `show_widget` widget, downscaling the image to fit under `Read`'s line cap, then reading the file back to re-embed it is exactly the time-wasting "фигня". Don't do any of it for a plain "покажи скриншот".) `Read`-ing a PNG only lets *Claude* see it, never the user.
- **The `show_widget` base64 dance below is ONLY for the narrow region/OCR-debugging case** — when candidate zones must be drawn ON the frame to confirm a bbox/reader fix. Never reach for it for a plain "show me the screen".
  - **ALWAYS draw the annotations ON TOP OF the real screenshot — never substitute a hand-drawn schematic.** (User rule, do not deviate: "рисуй поверх картинки реальной — не надо свое что то рисовать".) Embed the actual PNG/JPEG as a base64 `data:` URI in an `<img>`, then overlay the markup (SVG/HTML boxes, markers, labels, click targets) positioned over it. The visualize `read_me` suggests a schematic as a "cheaper/more reliable" alternative — that suggestion is OVERRIDDEN here; the user wants the real frame with annotations, not your redrawing of it.
  - Keep the embedded image small (tight crop, downscaled, JPEG quality ~70) so the base64 stays well under the widget size cap, but it must be the real captured frame.
  - **When debugging regions / OCR (bbox placement, screen_verify, readers): draw EVERY candidate zone as a labelled box ON the real frame AND annotate each box with WHAT IT RECOGNISED** (e.g. `rc_level [28,40.6,8.9,3.1] → ""` vs a corrected box `→ "19"`). (User rule: "рисуй зоны и тд + что распознал".) Never just paste raw crops or OCR numbers in text — the user must see the zone position and its read value together on the screenshot. This is how region fixes get confirmed.
