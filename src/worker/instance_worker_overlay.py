        ...
        *,
        ...
        if isinstance(pu, list):
            for item in pu:
                ...
                # A device-level push cancels the running task to make room (see
                # the ``is_device_level`` block below). If the SAME scenario is
                # already running, that cancel preempts it mid-run only to start
                # an identical instance (reason=preempted_by_device_level) — and
                # while its trigger region stays on screen the rule re-fires every
                # tick, so the scenario never finishes. Skip the push: the running
                # instance covers this frame, and once it ends the next tick that
                # still sees the trigger re-pushes cleanly.
                if is_device_level and t == (
                    getattr(self, "_current_task_type", None) or ""
                ):
                    handle = getattr(self, "_current_task_handle", None)
                    if handle is not None and not handle.done():
                        logger.debug(
                            "overlay: skipping push %s — already running on %s",
                            t, self._cfg.instance_id,
                        )
                        _record_push_scenario(
                            scenario=t, screen=set_node_snap, region=reg_snap,
