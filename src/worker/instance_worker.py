...
from coord import CoordWorkerMixin
    ...
    async def _seed_startup_tasks(self) -> None:
        ...
        if self._redis is not None:
            try:
                ...
                active_player = (
                    raw_ap.decode() if isinstance(raw_ap, bytes) else str(raw_ap or "")
                ).strip()
            except Exception:
                logger.debug("startup seed: active_player read failed", exc_info=True)
                active_player = ""
            ...
            try:
                while True:
                    ...
                    if now_m - last_heartbeat >= 2.0 and self._redis is not None:
                        ...
                        # Coordination layer: fleet heartbeat + fid→instance index.
                    ...
                    while self._ui_paused:
                        ...
                        if now_m - last_heartbeat >= 2.0 and self._redis is not None:
                            ...
                            # Keep the fleet view accurate while paused (online,
                            # not dead) — but don't drain directives until resumed.
                            await self._publish_fleet_heartbeat(time.time())
                            last_heartbeat = now_m
                        await self._drain_ui_commands()
                    ...
                    if item is None:
                        ...
                        if self._redis is not None:
                            ...
                            dir_key = self._coord_inbox_key()
                            raw_bp = await self._redis.brpop([cmd_key, dir_key], timeout=2)  # type: ignore[union-attr]
                            if raw_bp:
                                popped_key, payload = raw_bp
                                pk = (
                                    popped_key.decode()
                                    if isinstance(popped_key, bytes)
                                    else str(popped_key)
                                )
                                if pk == dir_key:
                                    await self._handle_directive_raw(payload)
                                    await self._drain_directives()
                                else:
