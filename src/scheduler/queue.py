...
# Ranking knobs (W_HOPS, W_RECENT, HOPS_DEBUFF_CAP_HOPS, UNREACHABLE_DEBUFF,
# HOPS_SENTINEL) and the scoring formula now live in ``scheduler.ranking`` and
# are imported above; they stay importable from this module for back-compat.
...
class RedisQueue:
    ...
    def _task_types_requiring_node() -> set[str]:
        ...
        Node-less recovery scenarios (``dismiss_unknown_popup``) carry no
        ``node:`` and are therefore *not* in this set — they remain free to run
        precisely when the screen is unknown, which is what bridges us back to a
        known screen. ``who_i_am`` is **not** node-less — it declares
        ``node: chief_profile`` (it navigates to the profile to OCR the id), so
        it *is* gated here and can only run once the screen is known again. The
        identity bootstrap therefore depends on the dismissers first clearing
        any blocking modal.
    ...
    def _task_type_to_required_node_cached(
        ...
        for resolved in iter_resolved_keys(repo_root_path):
            ...
            out[resolved.key] = allowed_nodes[0]
        return out
    ...
    def _rank_candidates(
        ...
        *,
        ...
        for raw, data in due:
            ...
            # Resolve hops here (not in ``compute_rank``) so the screen graph —
            # and the golden tests' ``_bfs_hops`` monkeypatch — stay in this
            # module. Only meaningful when both endpoints are known.
            hops: int | None = None
            if required_node and current_screen:
                hops = _bfs_hops(current_screen, required_node)

            sort_key, meta = compute_rank(
                base_priority=base,
                current_screen=current_screen,
