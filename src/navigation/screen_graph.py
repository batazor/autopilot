...
def _building_screen_defs(root: Path, *, game: str | None = None) -> list[tuple[str, str]]:
    """Building ids/names for synthetic building page routes and OCR verify rules."""
    from config.games import (
        base_game_for_module_catalog,
        modules_root_for,
        resolve_module_catalog,
    )

    catalog = resolve_module_catalog(game)
    if base_game_for_module_catalog(catalog) != "wos":
        return []
    buildings_dir = modules_root_for("wos", repo_root=root) / "db" / "buildings"
    index_path = buildings_dir / "index.yaml"
    if not index_path.is_file():
        return []
    raw = yaml.safe_load(index_path.read_text(encoding="utf-8")) or {}
    items = raw.get("buildings") if isinstance(raw, dict) else None
    ...
    for item in items:
        ...
        if not name and bid:
            file_rel = str(item.get("file") or "").strip() or f"{bid}.yaml"
            path = buildings_dir / file_rel
            if path.is_file():
...
def _load_edge_taps(
    ...
    * ``dict`` — dynamic edge resolved at runtime via an :data:`EDGE_RESOLVERS`
    ...
    # edge_taps.yaml entry still override any of these.
    #   heroes → page.heroes.<hid>       grid entry (dynamic, via hero_grid)
    #   page.heroes.<hid> → heroes       back
    #   page.heroes.<hid> → heroes.<hid>.wiki   open the wiki popup
    #   heroes.<hid>.wiki → page.heroes.<hid>   back from the popup
    from config.paths import repo_root

    for hid in _hero_ids():
        src_card = f"page.heroes.{hid}"
        dst_wiki = f"heroes.{hid}.wiki"
        if ("heroes", src_card) not in static:
            dynamic.setdefault(
                ("heroes", src_card), {"resolver": "hero_grid", "target": hid}
            )
        static.setdefault((src_card, "heroes"), ["icon.page.back"])
...
def _resolve_active_game() -> str:
    try:
        ...
        return get_active_module_catalog()
    except Exception:
        from config.games import default_module_catalog
...
def _area_screen_region_landmarks(root: Path) -> dict[str, list[VerifyRule]]:
    ...
    for entry in raw.get("screens") or []:
        ...
        if found_region and action == "text":
            # Area-level ``screen_region`` landmarks do not carry expected OCR
            # text. Treating a text region as a match rule would make any
            # non-empty OCR result prove the screen; explicit screen_verify
            # OCR rules should declare ``ocr`` + ``contains`` instead.
            continue
...
def _load_screen_verify_config_cached(
    ...
    # Synthesize per-building page verify rules from games/wos/db/buildings.
    # Each building is a CONCRETE screen node, detected directly by its name in
    # a title OCR region (e.g. "Cookhouse Lv. 1"). The old generic
    ...
    for building_id, building_name in building_defs:
        if building_id == "furnace":
            ocr_region = "furnace.name"
        elif building_id == "cookhouse":
            ocr_region = "building.name"
        elif building_id == "shelter":
