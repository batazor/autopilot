...
Example:
    uv run issue-license --email alice@example.com \\
        ...
        --out ./alice.licence.json

Default output path is ``./<email-slug>.licence.json`` so multiple
...
def _default_out_path(sub: str) -> Path:
    return Path.cwd() / f"{_slug_for_email(sub)}.licence.json"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="issue-license",
        description="Sign a license JWT and write it as a licence.json envelope.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="output path (default: ./<email-slug>.licence.json)",
    )
    parser.add_argument(
        "--stdout",
        action="store_true",
    ...
    print(f"  max_devices:         {envelope['max_devices']}")
    print(f"  max_players_/device: {envelope['max_players_per_device']}")
    print(f"  expires_at: {envelope['expires_at']}")
    print()
    print("Send this file to the user. They import it via the UI (/license)")
    print("or place it at <repo>/license-data/licence.json and restart the bot.")
    return 0
