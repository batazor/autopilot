...
- ``_last_match`` (:class:`tasks.dsl_match_result.MatchResult`) — sticky state
  used by the click executor to tap matched coords on the same region
    ...
    async def _match_region(
        ...
        *,
        ...
        if red_dot_req is not None:
            ...
            area_action = step_action or str(pair[1].get("action") or "").strip()
            if area_action not in {
                "exist",
                "text",
                "color_check",
                "findIcon",
            ...
            color = str(step.get("color") or step.get("cta_color") or "").strip()
            if color:
                rule["color"] = color
            ...
            x_padding_ratio = step.get("x_padding_ratio")
            if x_padding_ratio is not None:
                rule["x_padding_ratio"] = x_padding_ratio
        ...
        if isinstance(row, dict):
            ...
            self._last_match = MatchResult(region, row)
            await self._persist_dsl_last_match(
                instance_id,
                region=region,
                threshold=threshold,
