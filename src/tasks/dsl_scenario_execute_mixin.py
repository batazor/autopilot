...
class DslScenarioExecuteMixin(
    ...
    async def execute(self, instance_id: str) -> TaskResult:
        ...
        from dsl.dsl_schema import scenario_allowed_nodes, scenario_navigate_enabled
        ...
        # Opportunistic helpers (``navigate: false``) declare ``nodes:`` only to
        # say *where* they are meaningful — never to be driven there. The analyzer
        # pushes ``tabs.strip.advance`` from a Shop/Deals sub-page, but the queued
        # task can outlive the visit; popped on ``main_city`` it used to navigate
        # back into Shop purely to flip a tab (a pointless round-trip — the work
        # is moot the moment we leave the strip). Run the steps in place when
        # already on an allowed node; otherwise skip with a benign success rather
        # than travelling. ``target_node`` is zeroed on the in-place path so the
        # screen-identity gate and navigation block below are bypassed.
        if (
            allowed_nodes
            and not scenario_navigate_enabled(doc)
            and self.start_step_index <= 0
        ):
            cur_for_gate = await _read_current_screen(instance_id, self.redis_client)
            if cur_for_gate in allowed_nodes:
                target_node = ""
            else:
                await self._clear_step_context(instance_id)
                logger.info(
                    "dsl_scenario: skipping %s — off-node (%s) and navigate:false "
                    "(opportunistic helper does not travel)",
                    _scen(key),
                    cur_for_gate or "unknown",
                )
                _trace_row(
                    0,
                    {"navigate_to": ""},
                    "early_exit",
                    reason="off_node_navigate_disabled",
                    target=cur_for_gate,
                )
                return TaskResult(
                    success=True,
