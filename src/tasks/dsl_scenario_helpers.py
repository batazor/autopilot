...
async def _enqueue_scenario(
    *,
    ...
    enqueued = await queue.schedule(
        ...
        # Per-call payload (e.g. the daily-mission router passes the parsed
        # resource/troop/count). The queue stores it and the worker hydrates
        # ``DslScenarioTask.args`` from it. Queue dedup is by task_type only, so
        # two pushes of the same scenario with different args collapse to one —
        # callers that need both to survive must vary the scenario key.
        args=args or None,
    )
    logger.info(
