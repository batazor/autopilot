...
class DslScenarioTask(
    ...
    # instead of the static region center when `*_search` is involved. Region +
    # hit row are bundled in one frozen object (see tasks.dsl_match_result).
    _last_match: MatchResult | None = field(default=None, init=False, repr=False)
    # Same idea as ``_last_match`` but for ``ocr:`` steps — set by
