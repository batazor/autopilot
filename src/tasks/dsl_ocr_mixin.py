        ...
        *,
        ...
        if store_redis_field:
            if self.redis_client is None:
                ...
                # ``accumulate: true`` (string OCR only) appends each read to the
                # existing buffer instead of overwriting — the building block for
                # reading a scrolled list (``while_scroll`` + ``ocr``) into one
                # field. The scenario-start hook clears ``store:`` fields, so the
                # buffer always starts fresh per run. An exact-tail repeat (the
                # scroll-end detector re-reads the same last page) is skipped so
                # the buffer stays bounded.
                store_value = str(value)
                store_text = text
                if (
                    bool(step.get("accumulate"))
                    and type_hint not in {"int", "integer"}
                    and store_value
                ):
                    async def _prior(field: str) -> str:
                        try:
                            raw = await self.redis_client.hget(redis_key, field)
                        except Exception:
                            return ""
                        return (
                    ...
                    # Default the written value to the existing buffer so an
                    # exact-tail repeat (scroll-end re-reads the same page) keeps
                    # the buffer intact instead of clobbering it with the bare
                    # last page; only a genuinely new page is appended.
                    prior = await _prior(store_redis_field)
                    if prior:
                        store_value = (
                            prior if prior.endswith(str(value)) else f"{prior}\n{value}"
                        )
                    prior_text = await _prior(f"{store_redis_field}_text")
                    if prior_text:
                        store_text = (
                            prior_text
                            if not text or prior_text.endswith(text)
                            else f"{prior_text}\n{text}"
                        )

                mapping: dict[str, str] = {
                    store_redis_field: store_value,
                    f"{store_redis_field}_text": store_text,
