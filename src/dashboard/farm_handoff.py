...
clicks Sign Up, and the SDK register API responds. The dashboard can still send
**Failed** as a manual abort; legacy ``done`` signals are harmless acknowledgements.
...
def set_pending(client: Redis, username: str, **status: object) -> None:
    ...
    run's signal can't be mistaken for this one.
    """
...
def signal(client: Redis, username: str, outcome: str) -> None:
    """Record an operator side-channel for ``username`` ("failed" aborts)."""
