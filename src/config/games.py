...
def is_known_module_catalog(catalog: str) -> bool:
    """True if ``catalog`` can own module files under ``games/<catalog>``."""
    c = (catalog or "").strip()
    return c in GAMES or c in MODULE_CATALOG_OVERLAYS
...
def default_module_catalog() -> str:
    """Module catalog used when callers do not pass an explicit game/catalog."""
    try:
        ...
        catalog = ""
    return (catalog or DEFAULT_GAME).strip()


def resolve_module_catalog(game: str | None = None) -> str:
    """Resolve a call-site ``game`` argument into a module catalog id."""
    return (game or default_module_catalog()).strip() or DEFAULT_GAME


def base_game_for_module_catalog(catalog: str | None) -> str:
    """Gameplay id behind a module catalog."""
    c = (catalog or DEFAULT_GAME).strip()
    if c in MODULE_CATALOG_OVERLAYS:
        return MODULE_CATALOG_OVERLAYS[c][0]
    return c
...
def modules_root_for(game: str, repo_root: Path | None = None) -> Path:
    """Directory that holds ``<game>`` or module-catalog feature modules.
    ...
    if g in MODULE_CATALOG_OVERLAYS:
        g = MODULE_CATALOG_OVERLAYS[g][-1]
    root = (repo_root if repo_root is not None else default_repo_root()).resolve()
    return root / GAMES_DIR_NAME / g


def module_roots_for(game: str | None = None, repo_root: Path | None = None) -> tuple[Path, ...]:
    """Filesystem roots that compose a module catalog, in merge order."""
    catalog = resolve_module_catalog(game)
    roots = MODULE_CATALOG_OVERLAYS.get(catalog, (catalog,))
    return tuple(modules_root_for(root_id, repo_root=repo_root) for root_id in roots)
...
def modules_path_prefix(game: str | None = None) -> str:
    """Repo-relative prefix for ``game`` or a module catalog, e.g. ``games/wos``."""
    catalog = resolve_module_catalog(game)
    g = MODULE_CATALOG_OVERLAYS.get(catalog, (catalog,))[-1]
    return f"{GAMES_DIR_NAME}/{g}"


def module_path_prefixes(game: str | None = None) -> tuple[str, ...]:
    """Repo-relative prefixes that compose a module catalog."""
    catalog = resolve_module_catalog(game)
    roots = MODULE_CATALOG_OVERLAYS.get(catalog, (catalog,))
    return tuple(f"{GAMES_DIR_NAME}/{root_id}" for root_id in roots)
...
def is_module_reference(path: str) -> bool:
    ...
    Recognizes the Phase 3+ shape ``games/<module-catalog>/<module>/<rest>``.
    ...
    if len(parts) < 3 or parts[0] != GAMES_DIR_NAME:
        return False
    return is_known_module_catalog(parts[1])
...
def split_repo_relative(path: str) -> tuple[str, str] | None:
    ...
    isn't under any known module catalog tree or when the module-id segment
    is missing.
...
def split_game_module(path: str) -> tuple[str, str, str] | None:
    """Split a repo-relative reference path into ``(catalog, module_id, rest)``.
    ...
    if len(parts) < 3 or parts[0] != GAMES_DIR_NAME:
        return None
    game = parts[1]
    if not is_known_module_catalog(game):
        return None
    tail_parts = parts[2:]
