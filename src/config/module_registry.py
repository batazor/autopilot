...
def _module_context(
    ...
    *,
    ...
    return WikiModuleContext(
        ...
        game=resolve_module_catalog(game),
    )
...
def path_matches_module_scope(
    ...
    *,
    ...
    try:
        ...
        return False
    if scope == ALL_MODULES_KEY:
        return any(rel.startswith(f"{prefix}/") for prefix in module_path_prefixes(g))
    if scope == CORE_MODULE_KEY:
        return any(
            rel.startswith(f"{prefix}/{_module_discovery.CORE_MODULES_DIR}/")
            for prefix in module_path_prefixes(g)
        )
...
def ocr_path_belongs_to_context(ocr: str, ctx: WikiModuleContext) -> bool:
    ...
    if ctx.is_all:
        ...
        if not (is_module_reference(raw) and "/references/" in raw):
            return False
        return not ctx.game or any(
            raw.startswith(f"{prefix}/") for prefix in module_path_prefixes(ctx.game)
        )
