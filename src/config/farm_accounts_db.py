...
``state_sqlite`` / ``devices_db`` / ``giftcodes_db``. Credentials live in
``state.db`` like the rest of the app's persistent data.
"""
...
def _drop_legacy_fid_column(conn) -> None:  # noqa: ANN001 - sqlite connection
