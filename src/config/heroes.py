...
Source of truth: ``games/<game>/db/heroes/index.yaml`` and per-hero YAML —
the per-game reference DB, alongside ``db/buildings`` (see config.buildings).
"""
...
HEROES_DB_REL = Path("db") / "heroes"


def heroes_db_dir(repo_root: Path | None = None) -> Path:
    """Canonical per-game heroes reference DB (``games/<game>/db/heroes/``)."""
    root = (repo_root if repo_root is not None else default_repo_root()).resolve()
    return modules_root_for(default_game(), repo_root=root) / HEROES_DB_REL


def hero_index_path(repo_root: Path | None = None) -> Path:
    return heroes_db_dir(repo_root) / "index.yaml"


def hero_yaml_path(hero_id: str, repo_root: Path | None = None) -> Path:
    hid = (hero_id or "").strip()
    return heroes_db_dir(repo_root) / f"{hid}.yaml"
...
def load_heroes(repo_root: Path | None = None) -> HeroRegistry:
    heroes_dir = heroes_db_dir(repo_root)
