...
_ORB_MAX_FEATURES = 1000
_ORB_LOWE_RATIO = 0.75
_ORB_MIN_GOOD_MATCHES = 8
# ORB defaults (edgeThreshold=31, patchSize=31) wipe out all keypoints on
# small UI icon templates (the bulk of overlay rule crops are 30–100 px).
# Shrink both so feature_match remains usable on real icons.
_ORB_EDGE_THRESHOLD = 5
_ORB_PATCH_SIZE = 15
_orb_detector_lock = threading.Lock()
_orb_detector: cv2.ORB | None = None


def _get_orb() -> cv2.ORB:
    """Lazy, thread-safe singleton — ORB_create allocates on every call."""
    global _orb_detector
    with _orb_detector_lock:
        if _orb_detector is None:
            _orb_detector = cv2.ORB_create(
                nfeatures=_ORB_MAX_FEATURES,
...
def _match_orb_features_in_bbox(
    image_bgr: np.ndarray,
    template_bgr: np.ndarray,
    search_bbox: dict[str, float],
) -> TemplateMatchResult:
    """ORB + Lowe-ratio + RANSAC homography: locate ``template_bgr`` inside the bbox.

    Returns the same ``TemplateMatchResult`` shape as :func:`_match_direct_template_in_bbox`
    so the downstream click / red-dot / while_match plumbing works unchanged.
    ``score`` is the inlier ratio in ``[0, 1]`` (RANSAC inliers / good matches).
    """
    search, (left, top) = patch_bgr_from_bbox_percent(image_bgr, search_bbox)
    tw = int(template_bgr.shape[1])
    th = int(template_bgr.shape[0])
    ...
    empty = TemplateMatchResult(
        ...
        score_ncc=0.0,
        score_ncc_second=None,
        match_source="orb_features",
        hash_distance=None,
        template_w=tw,
        template_h=th,
    )
    if search.size == 0 or template_bgr.size == 0:
        return empty

    orb = _get_orb()
    tpl_gray = cv2.cvtColor(template_bgr, cv2.COLOR_BGR2GRAY)
    search_gray = cv2.cvtColor(search, cv2.COLOR_BGR2GRAY)
    kp_t, des_t = orb.detectAndCompute(tpl_gray, None)
    kp_s, des_s = orb.detectAndCompute(search_gray, None)
    if des_t is None or des_s is None or len(kp_t) < 2 or len(kp_s) < 2:
        return empty

    bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=False)
    pair_matches = bf.knnMatch(des_t, des_s, k=2)
    good = [
        m for pair in pair_matches
        if len(pair) == 2
        for m, n in [pair]
        if m.distance < _ORB_LOWE_RATIO * n.distance
    ]
    if len(good) < _ORB_MIN_GOOD_MATCHES:
        return empty
    ...
    if inliers < _ORB_MIN_GOOD_MATCHES:
        return empty
    ...
    score = float(inliers) / float(len(good))
    return TemplateMatchResult(
        score=score,
    ...
    for compiled in plan:
        ...
        if action == "feature_match":
            ...
            if not isinstance(bbox, dict):
                out[logical_name] = {"matched": False, "reason": "missing_bbox"}
                continue
            direct_template = compiled.direct_template
            if not direct_template:
                out[logical_name] = {
                    "matched": False,
                    "reason": "feature_match_requires_template",
                }
                continue
            ...
            tpl_bgr, _tpl_mask = tpl_pair
            res = _match_orb_features_in_bbox(image_bgr, tpl_bgr, search_bbox)
            ...
            tl_orb = (int(res["top_left"][0]), int(res["top_left"][1]))
            ...
            if isinstance(rule.get("isRedDot"), bool):
                hit_fm["red_dot_required"] = rule["isRedDot"]
                hit_fm["red_dot_present"] = bool(red_dot_present)
                hit_fm["red_dot_search_mode"] = "dynamic"
                if red_dot_bbox is not None:
                    hit_fm["red_dot_bbox"] = red_dot_bbox
            if compiled.push_tasks:
                hit_fm["pushScenario"] = compiled.push_tasks
            if set_node_s:
                hit_fm["set_node"] = set_node_s
            if priority is not None:
                hit_fm["priority"] = priority
            out[logical_name] = hit_fm
            continue
