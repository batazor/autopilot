...
def _list_scenario_files(repo_root: Path, module_scope: str) -> list[ScenarioFile]:
    ...
    """
    resolved = _tmpl.iter_resolved_keys(repo_root)
    if not resolved:
        return []
    out: list[ScenarioFile] = []
    for rk in resolved:
        if not path_matches_module_scope(rk.path, repo_root, module_scope):
            continue
        try:
        ...
        out.append(
            ScenarioFile(
                path=rk.path,
                rel_scenarios=scenario_source_label(rk.path, repo_root),
...
files = _list_scenario_files(repo_root, module_scope)
if not files:
    st.warning(
