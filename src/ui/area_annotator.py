...
class AreaEntryDict(TypedDict, total=False):
    ...
    screen_id: str
    screen_region: str
    """Optional region name that identifies this entry's ``screen_id`` at runtime."""
    regions: list[RegionDict]
...
def _render_screen_id_and_ocr_fields(
    ...
    *,
    ...
    try:
        ...
        region_index = 0
    region_key = f"screen_region_{entry_idx}_{_sk}"
    if region_key in st.session_state:
        state_region = str(st.session_state.get(region_key, "") or "").strip()
        if state_region not in region_opts:
            st.session_state[region_key] = region_default if region_default in region_opts else ""
        region_select_index = None
    else:
        region_select_index = region_index
    if sid:
        screen_region = st.selectbox(
            "Screen identity region",
            options=region_opts,
            index=region_select_index,
            format_func=_format_screen_region_choice,
            key=region_key,
            help=(
                "Optional region that proves this reference image is the selected node. "
                "Detector uses it as a template landmark for current_screen."
            ),
        )
        screen_region_s = str(screen_region).strip()
        if screen_region_s:
            cur["screen_region"] = screen_region_s
        else:
            cur.pop("screen_region", None)
    else:
        cur.pop("screen_region", None)
    ...
    with st.expander("Regions", expanded=True):
        ...
        def _on_region_pick_change(pkey: str = pick_key) -> None:
            ...
            if isinstance(raw, int):
                _apply_region_pick_idx(int(raw), regs_now)
                return
            if isinstance(raw, str):
                head = raw.split(":", 1)[0].strip()
                if head.isdigit():
                    _apply_region_pick_idx(int(head), regs_now)

        target_idx = _resolve_selected_region_idx(regions)
        if labeling_mode:
            filter_key = f"area_region_filter_{_rk}"
            ...
            visible = [
                i
                for i, r in enumerate(regions)
                if not filter_q or filter_q in str(r.get("name", "")).lower()
            ]
            if not visible:
                st.caption("No regions match the filter.")
                visible = list(range(len(regions)))
            pick_options = visible
            pick_labels = {i: names[i] for i in pick_options}
