...
def module_streamlit_pages_by_nav(repo_root: Path) -> dict[str, list[StreamlitPage]]:
    """``{nav_group: [st.Page, ...]}`` for all registered module UI pages."""

    grouped: dict[str, list[StreamlitPage]] = {}
...
def extend_nav_pages(
    nav: dict[str, list[StreamlitPage]],
    module_pages: dict[str, list[StreamlitPage]],
