...
def test_solve_returns_none_affine_without_corners() -> None:
    """Backward compatibility: no constraints → positions + a None affine."""
    from modules.radar.stitch import _solve_matched_positions

    images = [np.zeros((2, 2, 3), np.uint8), np.zeros((2, 2, 3), np.uint8)]
    edges = [MatchEdge(i=0, j=1, dx=10.0, dy=5.0, score=1.0)]
    positions, affine = _solve_matched_positions([(0.0, 0.0), (10.0, 5.0)], images, edges)
    assert affine is None
    assert len(positions) == 2


def test_solve_with_corner_constraints_recovers_affine_and_dedrifts() -> None:
    """The joint solve recovers the game→canvas affine from 4 marked corners and
    places each corner-frame so its corner lands on the square game grid even
    when the nominal layout starts drifted."""
    from modules.radar.stitch import _solve_matched_positions

    lin = np.array([[2.0, 0.4], [-0.4, 2.0]])  # isometric-ish: scale + shear
    off = np.array([300.0, 200.0])
    games = [(0.0, 0.0), (1000.0, 0.0), (1000.0, 1000.0), (0.0, 1000.0)]
    fpx = (120.0, 90.0)  # the corner's position inside each frame
    true_pos = [tuple(lin @ np.array(g) + off - np.array(fpx)) for g in games]
    edges = [
        MatchEdge(
            i=k, j=k + 1,
        ...
        for k in range(len(true_pos) - 1)
    ]
    images = [np.zeros((2, 2, 3), np.uint8) for _ in true_pos]
    drift = [(0.0, 0.0), (15.0, -8.0), (30.0, 12.0), (-10.0, 20.0)]
    nominal = [(true_pos[k][0] + drift[k][0], true_pos[k][1] + drift[k][1]) for k in range(4)]
    constraints = [(k, fpx, games[k]) for k in range(4)]

    positions, affine = _solve_matched_positions(nominal, images, edges, constraints)

    assert affine is not None
    rlin, roff = affine
    assert np.allclose(rlin, lin, atol=0.02)
    # Every marked corner lands on the affine grid (drift removed, sub-pixel).
    for k in range(4):
        cx, cy = positions[k][0] + fpx[0], positions[k][1] + fpx[1]
        ax = rlin[0][0] * games[k][0] + rlin[0][1] * games[k][1] + roff[0]
        ay = rlin[1][0] * games[k][0] + rlin[1][1] * games[k][1] + roff[1]
        assert abs(cx - ax) < 1.0
        assert abs(cy - ay) < 1.0
