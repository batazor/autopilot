...
def _swipe_relative(
    ...
    if cfg.raster is not None:
        # Raster deltas are already SCREEN px (no minimap diamond to scale
        # through), so the finger travel is the step itself — autoscale still
        # learns the true px-moved-per-px-swiped from measured offsets.
        finger_dx = -minimap_dx * nav.swipe_scale
        finger_dy = -minimap_dy * nav.swipe_scale
    else:
