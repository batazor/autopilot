...
# Corner constraint weight in the joint position+affine solve. High enough that
# the 4 marked corners pull their frames onto the square game grid (drift
# removed) yet still least-squares-balanced across them. Tuned on-device.
CORNER_CONSTRAINT_WEIGHT = 6.0

# (linear 2×2, offset) mapping game coords → solve-space canvas positions.
SolveAffine = tuple[tuple[tuple[float, float], tuple[float, float]], tuple[float, float]]
# (frame_index, frame_px, game_xy) — a marked corner pinned to its game vertex.
CornerConstraint = tuple[int, tuple[float, float], tuple[float, float]]
...
def _solve_matched_positions(
    ...
    images: list[np.ndarray | None],
    edges: list[MatchEdge],
    corner_constraints: list[CornerConstraint] | None = None,
) -> tuple[list[tuple[float, float]], SolveAffine | None]:
    """Solve frame canvas positions from measured pair offsets.

    With ``corner_constraints`` (``[(frame_index, frame_px, game_xy), ...]``) the
    game→canvas affine is solved JOINTLY with the positions: each axis gains 3
    affine unknowns (``a,b,c`` for x, ``d,e,f`` for y) and one row per corner —
    ``pos[f].x + frame_px.x == a·gx + b·gy + c`` — so the marked corners pull
    their frames onto the square game grid (accumulated drift is spread out by
    the edge graph) and the returned affine maps game coordinates onto these
    positions exactly. Returns ``(positions, affine)``; ``affine`` is ``None``
    without constraints (the legacy drift-carrying behaviour).
    """
    valid = [idx for idx, img in enumerate(images) if img is not None]
    if len(valid) < 2 or not edges:
        return nominal_positions, None

    index = {idx: row for row, idx in enumerate(valid)}
    n = len(valid)
    use_corners = bool(corner_constraints)
    # Extra columns a,b,c (x-solve) / d,e,f (y-solve): the same matrix serves
    # both axes, only the right-hand sides differ (frame_px.x vs frame_px.y).
    ncols = n + (3 if use_corners else 0)
    ...
    if use_corners:
        for frame_idx, (fpx_x, fpx_y), (gx, gy) in corner_constraints:  # type: ignore[union-attr]
            if frame_idx not in index:
                continue
            # pos[f] − (a·gx + b·gy + c) == −frame_px  (per axis)
            row = [0.0] * ncols
            row[index[frame_idx]] = 1.0
            row[n], row[n + 1], row[n + 2] = -float(gx), -float(gy), -1.0
            rows.append(row)
            bx.append(-float(fpx_x))
            by.append(-float(fpx_y))
            weights.append(CORNER_CONSTRAINT_WEIGHT)
    ...
    affine = None
    if use_corners:
        # linear = [[a, b], [d, e]] ; offset = [c, f] — maps game → solve-coords.
        affine = (
            ((float(solved_x[n]), float(solved_x[n + 1])), (float(solved_y[n]), float(solved_y[n + 1]))),
            (float(solved_x[n + 2]), float(solved_y[n + 2])),
        )
    return positions, affine
