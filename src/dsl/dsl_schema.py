...
class DslStep(BaseModel):
    ...
    * **Bare group** — only ``steps`` with no action key and no ``cond``: the
    ...
    # OCR-only (string ``store:`` targets): append each read to the existing
    # Redis buffer instead of overwriting. Used with ``while_scroll`` to read a
    # scrolled list (e.g. the full daily-mission list) into one field across
    # pages. The scenario-start hook clears ``store:`` fields, so the buffer
...
def scenario_navigate_enabled(doc: dict[Any, Any] | Any) -> bool:
    """Whether a ``node:``/``nodes:`` scenario may drive the FSM to reach its
    allowed nodes (default ``True``).

    Set ``navigate: false`` on an *opportunistic* helper whose work only makes
    sense when the bot is already on one of its nodes. ``tabs.strip.advance`` is
    the motivating case: the analyzer pushes it only from a Shop/Deals sub-page,
    but the queued task can outlive the visit. Popped on ``main_city`` it used to
    navigate back into Shop purely to flip a tab — a pointless round-trip, since
    advancing a strip is moot the moment we leave it. With ``navigate: false``
    the runtime runs the steps in place when already on an allowed node and skips
    with a benign success otherwise — it never travels to reach the nodes.

    Only an explicit boolean ``false`` disables navigation; any other value
    (absent, non-bool) keeps the default navigate-to-reach behaviour.
    """
    if not isinstance(doc, dict):
        return True
    val = doc.get("navigate", True)
