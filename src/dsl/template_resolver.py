...
def _hero_index(repo_root_s: str) -> dict[str, str]:
    """``hero_id → display name`` from ``games/wos/db/heroes/index.yaml``."""
...
@lru_cache(maxsize=4)
def _building_index(repo_root_s: str) -> dict[str, str]:
    """``building_id → display name`` from ``games/wos/db/buildings/index.yaml``.

    Same shape as :func:`_hero_index` but for the ``building`` axis. Every id in
    the index becomes a concrete navigation node (``screen_graph`` synthesises a
    per-building screen + ``chapter.task`` edge from this same file), so index
    membership is the authority for a valid ``building.upgrade_<id>`` fill.
    """
    from config.games import modules_root_for

    idx_path = modules_root_for("wos", repo_root=Path(repo_root_s)) / "db" / "buildings" / "index.yaml"
    try:
        raw = yaml.safe_load(idx_path.read_text(encoding="utf-8")) or {}
    except (OSError, yaml.YAMLError):
        return {}
    out: dict[str, str] = {}
    for entry in raw.get("buildings") or []:
        if not isinstance(entry, dict):
            continue
        bid = str(entry.get("id") or "").strip()
        if not bid:
            continue
        name = str(entry.get("name") or bid).strip() or bid
        out[bid] = name
    return out
...
def _axis_context(repo_root: Path, axis: str, value: str) -> dict[str, str] | None:
    ...
    if axis == "building":
        index = _building_index(str(repo_root))
        name = index.get(value)
        if name is None:
            return None
        return {"building": value, "building_name": name}
    return None
    ...
    for root in _scenario_roots(repo_root):
        ...
        for p in templates:
            ...
            if axes == ["hero"]:
                ...
                for bid, bname in _building_index(str(repo_root)).items():
                    concrete_key = p.stem.replace("{building}", bid)
                    if concrete_key in seen:
                        continue
                    seen.add(concrete_key)
                    out.append(
                        ResolvedKey(
                            key=concrete_key,
                            path=p,
                            context={"building": bid, "building_name": bname},
                        )
                    )
    return out
