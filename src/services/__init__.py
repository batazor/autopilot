...
def get_active_module_catalog() -> str:
    """Active module catalog for this process, or active game/default."""
    from config.games import default_game

    catalog = _state.get(_K_ACTIVE_MODULE_CATALOG)
    if isinstance(catalog, str) and catalog:
        return catalog
    game = _state.get(_K_ACTIVE_GAME)
