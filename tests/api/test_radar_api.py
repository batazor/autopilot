...
def immediate_scan_calls(monkeypatch):
    ...
    def fake_run_scan(run_id, instance_id, client):
        calls.append((run_id, instance_id, client))
...
""".lstrip(),
    ...
    monkeypatch.setattr(radar, "_radar_config_path", lambda: path)
    return path
...
class TestScan:
    def test_missing_config_rejected_before_background_scan(
        ...
        monkeypatch.setattr(radar, "_radar_config_path", lambda: tmp_path / "missing.yaml")
    ...
    def test_start_sets_guard_and_runs_scan_immediately(
        ...
        assert immediate_scan_calls == [(run_id, instance_id, fake_redis)]
