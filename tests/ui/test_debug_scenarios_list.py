...
_MAIL_SCENARIO = REPO / "modules" / "mail" / "scenarios" / "mail.claim.yaml"


def test_scenario_source_label_for_module_path() -> None:
    if not _MAIL_SCENARIO.is_file():
        pytest.skip("modules/mail/scenarios/mail.claim.yaml missing — module not in tree")
    assert (
        scenario_source_label(_MAIL_SCENARIO, REPO)
        == "modules/mail/scenarios/mail.claim.yaml"
    )
...
def test_list_scenario_files_includes_module_mail_scenario() -> None:
    ...
    mail = next((sf for sf in files if sf.key == "mail.claim"), None)
    assert mail is not None
    assert mail.rel_scenarios == "modules/mail/scenarios/mail.claim.yaml"
    assert mail.repo_rel == "modules/mail/scenarios/mail.claim.yaml"
