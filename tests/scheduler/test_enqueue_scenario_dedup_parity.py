...
@pytest.mark.asyncio
async def test_enqueue_scenario_forwards_args_payload(
    redis_async: aioredis.Redis,
) -> None:
    ...
    ok = await _enqueue_scenario(
        ...
        run_at=1_700_000_000.0,
        skip_if_duplicate=True,
        args={"resource": "Meat", "target": 50000},
    )
    assert ok is True

    items = await _queue_payloads(redis_async, "bs1")
    assert len(items) == 1
    assert items[0]["args"] == {"resource": "Meat", "target": 50000}
