"""Watchdog launches by importing the module's ``main()`` via ``-c``.

The module path appears in argv either way, so process detection
(``is_health_watchdog_process``) works for the ``-c`` form and any legacy
``-m`` process still running.
"""
