...
def test_area_screen_region_skips_bare_text_landmark(mocker, tmp_path: Path) -> None:
    cfg = tmp_path / "screen_verify.yaml"
    cfg.write_text("screens: []\n", encoding="utf-8")
    _seed_fake_module(
        tmp_path,
        {
            "screens": [
                {
                    "screen_id": "text_screen",
                    "screen_region": "text.title",
                    "regions": [
                        {
                            "name": "text.title",
                            "action": "text",
    ...
    try:
        ...
        assert screen_graph.screen_landmark_rules("text_screen") == []
    finally:
        screen_graph.load_screen_verify_config.cache_clear()  # ty: ignore[unresolved-attribute]
        module_discovery._clear_module_discovery_caches()
        area_manifest.clear_area_doc_cache()
