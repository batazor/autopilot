...
from navigation.screen_graph import route_hops_async, route_taps, screen_verify_rules
...
def test_wos_beta_account_switch_routes() -> None:
    assert route_taps("main_city", "account_menu", game="wos_beta") == [
        ["main_city.to.account_menu"]
    ]
    assert route_taps("account_menu", "switch_menu", game="wos_beta") == [
        ["account_menu.to.switch_menu"]
    ]
    assert route_taps("main_city", "account_menu", game="wos") is None
