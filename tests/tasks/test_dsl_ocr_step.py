...
@pytest.mark.asyncio
async def test_ocr_accumulate_appends_pages_into_one_buffer(
    tmp_path: Path,
    mocker,
    redis_async: object,
) -> None:
    """``accumulate: true`` appends each read to the store buffer instead of
    overwriting — the building block for reading a scrolled list. An exact-tail
    repeat (scroll-end re-reads the last page) is skipped so the buffer stays
    bounded."""
    scenario_root = _scenario_root(tmp_path)
    (scenario_root / "by_cron").mkdir(parents=True)
    (scenario_root / "by_cron" / "read_list.yaml").write_text(
        yaml.dump(
            {
                "enabled": True,
                "name": "Read list",
    ...
    actions = make_actions(np.zeros((100, 200, 3), dtype=np.uint8))
    # Two distinct pages then a repeat of the last (mimics the scroll-end re-read).
    seq = ["alpha", "beta", "beta", "beta"]
    calls = {"n": 0}

    class _PagedOcrClient:
        async def ocr_region(
            self, image: np.ndarray, region: LayoutRegion, **_kwargs: Any
        ) -> OCRResult:
            text = seq[min(calls["n"], len(seq) - 1)]
            calls["n"] += 1
            return OCRResult(region_id="r0", text=text, confidence=0.97)

    import ocr.client as ocr_client_module

    mocker.patch.object(ocr_client_module, "OcrClient", _PagedOcrClient)
    patch_dsl(mocker, actions, repo_root=tmp_path)

    task = dsl.DslScenarioTask(
        task_id="t-accum",
        player_id="player_42",
        scenario_key="read_list",
        redis_client=redis_async,  # type: ignore[arg-type]
    )
    result = await task.execute("bs1")
    ...
    assert result.success is True
    assert calls["n"] >= 2
    buf = await redis_async.hget("wos:instance:bs1:state", "list.buf")  # type: ignore[attr-defined]
    # alpha appended, beta appended, subsequent beta(s) skipped (prior tail == "beta").
    assert buf == "alpha\nbeta"
...
async def test_exec_sync_building_name_persists_detected_level(
    ...
    instance_final = await redis_async.hgetall("wos:instance:bs1:state")  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]
    assert instance_final["current_screen"] == "cookhouse"
    assert instance_final["building.name"] == "Cookhouse Lv. 1"
    assert instance_final["building.name.parsed_id"] == "cookhouse"
...
@pytest.mark.asyncio
async def test_exec_sync_building_name_accepts_furnace_name_fallback(
    mocker,
    redis_async: object,
) -> None:
    import tasks.dsl_exec as dsl_exec
    ...
    mocker.patch.object(
        sync_state,
        "get_building_registry",
        side_effect=lambda: BuildingRegistry(
            buildings=(BuildingDef(id="furnace", name="Furnace"),)
        ),
    )
    mocker.patch.object(sync_state, "get_state_store", side_effect=lambda: _FakeStore())
    ...
    assert player_state["buildings.levels.furnace"] == "5"
    assert player_state["building.name.parsed_id"] == "furnace"
    assert instance_state["current_screen"] == "furnace"
    assert instance_state["building.name"] == "Furnace Lv. 5"


@pytest.mark.asyncio
async def test_exec_sync_furnace_level_mirrors_chief_profile_ocr(
    mocker,
    redis_async: object,
) -> None:
    import tasks.dsl_exec as dsl_exec
    ...
    await redis_async.hset(  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]
        "wos:player:player_42:state",
        mapping={"furnace.level": "30"},
    )

    await dsl_exec.DSL_EXEC_REGISTRY["sync_furnace_level"](
        dsl_exec.DslExecContext(
            redis_client=redis_async,
            player_id="player_42",
            instance_id="bs1",
    ...
    player_state = await redis_async.hgetall("wos:player:player_42:state")  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]
    instance_state = await redis_async.hgetall("wos:instance:bs1:state")  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]
    for state in (player_state, instance_state):
        assert state["furnace.level"] == "30"
        assert state["buildings.furnace.level"] == "30"
        assert state["buildings.levels.furnace"] == "30"
        assert state["furnace.level.synced_at"]
    assert captured == {
        "player_id": "player_42",
        "flat": {
            "buildings.furnace.level": 30,
            "buildings.levels.furnace": 30,
        },
    }


@pytest.mark.asyncio
async def test_exec_sync_building_name_parses_shelter_instance_number(
    mocker,
    redis_async: object,
) -> None:
    import tasks.dsl_exec as dsl_exec
    ...
    mocker.patch.object(
        ...
        "get_building_registry",
        side_effect=lambda: BuildingRegistry(
            buildings=(BuildingDef(id="shelter", name="Shelter"),)
        ),
    )
    mocker.patch.object(sync_state, "get_state_store", side_effect=lambda: _FakeStore())
...
async def test_exec_sync_hero_unit_persists_name_and_level(
    ...
    from games.wos.heroes.heroes import sync_hero_unit as hero_exec
    ...
    mocker.patch.object(hero_exec, "get_state_store", side_effect=lambda: _FakeStore())
...
async def test_ocr_chief_profile_player_id_against_real_tesseract() -> None:
    """Real-OCR sanity check: the labelled `chief_profile.png` shows player_id 2721690.
