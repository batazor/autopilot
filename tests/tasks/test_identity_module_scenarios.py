...
def test_who_i_am_resolves_player_id_before_player_state_writes() -> None:
    ...
    assert all(s["preprocess"] == "fast_digits" for s in ocr_steps[:3])
    # ``fast_digits`` (digit char-whitelist) disables Tesseract's language model
    # and collapses word-confidence to ~0 even on correct reads, so confidence is
    # floored at 0 here — the digit whitelist + ``type: integer`` + ``min_digits``
    # are the real guards. WOS player IDs are 7–10 digits (newer accounts are 7),
    # so ``min_digits`` floors at 7. See who_i_am.yaml.
    assert ocr_steps[0]["threshold"] == 0.0
    assert ocr_steps[0]["min_digits"] == 7
    assert ocr_steps[1]["threshold"] == 0.45
    assert ocr_steps[2]["store"] == "furnace.level"
    assert ocr_steps[2]["threshold"] == 0.0
    assert "player.power" not in {s["ocr"] for s in ocr_steps}

    exec_steps = [s for s in steps if isinstance(s, dict) and "exec" in s]
    assert [s["exec"] for s in exec_steps[:2]] == ["sync_furnace_level", "fetch_player"]
