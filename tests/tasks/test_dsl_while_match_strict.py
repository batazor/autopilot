...
@pytest.mark.asyncio
async def test_while_match_forwards_explicit_action_override(
    tmp_path: Path,
    mocker,
    redis_async: object,
    pin_click_to_center: None,
    ...
    raw["steps"][0]["action"] = "cta_button"
    raw["steps"][0]["color"] = "green"
    raw["steps"][0]["threshold"] = 0.5
    raw["steps"][0]["min_fill_ratio"] = 0.4
    raw["steps"][0]["max"] = 1
    yaml_path.write_text(yaml.dump(raw), encoding="utf-8")

    actions = make_actions([_frame_with_pattern(), _frame_with_pattern()])
    patch_dsl(mocker, actions, repo_root=tmp_path)
    mocker.patch.object(dsl, "click_approval_enabled", return_value=False)
    _patch_instant_sleep(mocker)
    seen_rules: list[dict[str, Any]] = []
    ...
    assert result.success is True
    assert seen_rules
    assert seen_rules[0]["action"] == "cta_button"
    assert seen_rules[0]["color"] == "green"
    assert seen_rules[0]["threshold"] == 0.5
    assert seen_rules[0]["min_fill_ratio"] == 0.4
