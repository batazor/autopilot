...
def test_wos_beta_module_dirs_merge_base_and_overlay(tmp_path: Path) -> None:
    from config.module_discovery import iter_module_dirs, module_storage_key

    wos_root = _modules_root_for("wos", repo_root=tmp_path)
    beta_root = _modules_root_for("wos_beta", repo_root=tmp_path)
    base = wos_root / "core" / "main_city"
    base_overlay = beta_root / "core" / "main_city"
    beta_only = beta_root / "events" / "beta_test"
    disabled_base = wos_root / "events" / "old_event"
    disabled_overlay = beta_root / "events" / "old_event"
    for mod in (base, base_overlay, beta_only, disabled_base, disabled_overlay):
        mod.mkdir(parents=True)
    (base / "module.yaml").write_text("id: main_city\n", encoding="utf-8")
    (base_overlay / "module.yaml").write_text("id: main_city\n", encoding="utf-8")
    (beta_only / "module.yaml").write_text("id: beta_test\n", encoding="utf-8")
    (disabled_base / "module.yaml").write_text("id: old_event\n", encoding="utf-8")
    (disabled_overlay / "module.yaml").write_text(
        "id: old_event\nenabled: false\n", encoding="utf-8"
    )

    dirs = list(iter_module_dirs(tmp_path, game="wos_beta"))

    assert base in dirs
    assert base_overlay in dirs
    assert beta_only in dirs
    assert disabled_base not in dirs
    assert disabled_overlay not in dirs
    assert dirs.index(base) < dirs.index(base_overlay)
    assert module_storage_key(base, tmp_path, game="wos_beta") == "wos_beta:core/main_city"
    assert (
        module_storage_key(base_overlay, tmp_path, game="wos_beta")
        == "wos_beta:core/main_city"
    )
    assert module_storage_key(beta_only, tmp_path, game="wos_beta") == "wos_beta:events/beta_test"


def test_wos_beta_account_switch_module_is_beta_only() -> None:
    from config.module_discovery import iter_module_dirs, module_meta_id, module_storage_key
    from config.paths import repo_root

    root = repo_root()
    wos_dirs = list(iter_module_dirs(root, game="wos"))
    beta_dirs = list(iter_module_dirs(root, game="wos_beta"))
    wos_ids = {module_meta_id(module_dir) for module_dir in wos_dirs}
    beta_by_id = {module_meta_id(module_dir): module_dir for module_dir in beta_dirs}

    assert "account/switch" not in wos_ids
    assert "account/switch" in beta_by_id
    assert (
        module_storage_key(beta_by_id["account/switch"], root, game="wos_beta")
        == "wos_beta:account/switch"
    )
