...
def test_modules_root_for_returns_games_root(tmp_path: Path) -> None:
    ...
    assert (
        modules_root_for("wos_beta", repo_root=tmp_path)
        == tmp_path.resolve() / "games" / "wos" / "beta"
    )


def test_wos_beta_module_catalog_overlays_wos(tmp_path: Path) -> None:
    assert WOS_BETA_MODULE_CATALOG == "wos_beta"
    assert MODULE_CATALOG_OVERLAYS["wos_beta"] == ("wos", "wos/beta")
    assert resolve_module_catalog("wos_beta") == "wos_beta"
    assert base_game_for_module_catalog("wos_beta") == "wos"
    assert is_known_module_catalog("wos_beta")
    assert not is_known_game("wos_beta")
    assert module_roots_for("wos_beta", repo_root=tmp_path) == (
...
def test_is_module_reference_accepts_repo_relative_module_paths() -> None:
    ...
    assert is_module_reference("games/wos/beta/core/heroes/references/page.png")
...
def test_split_repo_relative_extracts_module_id_and_tail() -> None:
    ...
    assert split_repo_relative("games/wos/beta/core/heroes/references/p.png") == (
        "beta/core/heroes",
        "references/p.png",
    )
...
def test_split_game_module_includes_game_segment() -> None:
    ...
    assert split_game_module("games/wos/beta/core/heroes/area.yaml") == (
        "wos",
        "beta/core/heroes",
        "area.yaml",
    )
...
def test_modules_path_prefix_returns_two_segment_prefix() -> None:
    ...
    assert modules_path_prefix("wos_beta") == "games/wos/beta"
