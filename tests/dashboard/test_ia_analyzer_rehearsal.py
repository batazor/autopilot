...
def test_ia_overlay_pusher_has_bot_actions(mocker) -> None:
    actions = mocker.Mock()
    mocker.patch.object(ia_overlay_executor, "get_bot_actions", return_value=actions)

    pusher = ia_overlay_executor._make_overlay_pusher()

    assert pusher._bot_actions is actions
