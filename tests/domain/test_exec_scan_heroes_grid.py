...
from games.wos.heroes.heroes import scan_heroes_grid
...
async def test_scan_heroes_grid_persists_full_snapshot_and_positions(
    ...
    monkeypatch.setattr(scan_heroes_grid, "get_state_store", lambda: store)
    ...
    assert 0 < await redis_async.ttl(pos_key) <= scan_heroes_grid._HERO_GRID_POSITIONS_TTL_SECONDS
...
async def test_scan_heroes_grid_enqueues_red_dot_hero_scenarios(
    ...
    (the ``games/wos/heroes/heroes/scenarios/{hero}.yaml`` template keys) for the active
