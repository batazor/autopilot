...
uv run python -m notify_monitor                 # -> http://127.0.0.1:8800
uv run python -m notify_monitor --port 8800 --reload   # dev auto-reload
```
...
| `NM_DB_PATH`    | `notify_monitor/data/notify_monitor.db` | SQLite file            |
| `NM_LOG_PATH`   | `notify_monitor/data/notify_monitor.log` | log file (also console) |
...
```sh
uv run pytest notify_monitor/tests/ -q
```
