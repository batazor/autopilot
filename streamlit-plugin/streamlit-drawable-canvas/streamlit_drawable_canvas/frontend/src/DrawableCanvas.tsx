...
const DrawableCanvas = ({ args }: ComponentProps) => {
  ...
  // wos-fork: name of the region currently selected on the canvas, mirrored
  // back to Streamlit so the host UI can keep its region list in sync.
  ...
  useEffect(() => {
    if (!isEqual(initialState, initialDrawing)) {
      canvas.loadFromJSON(initialDrawing, () => {
        canvas.renderAll()
        // wos-fork: auto-select object tagged with `wos_active: true`
        // so resize handles appear immediately after a region is picked in
        // the regions list, without an extra click on the canvas.
        const target = canvas
          .getObjects()
          .find((o: any) => o && o.wos_active === true)
        if (target) {
  ...
  useEffect(() => {
    if (shouldReloadCanvas) {
      canvas.loadFromJSON(currentState, () => {
        // wos-fork: re-apply the auto-select after a reset/undo/redo reload,
        // otherwise the active object from the initial loadFromJSON gets
        // dropped by this second load (resetState triggers shouldReloadCanvas).
        const target = canvas
          .getObjects()
          .find((o: any) => o && o.wos_active === true)
        if (target) {
  ...
  useEffect(() => {
    ...
    // wos-fork: forward fabric selection changes back to Streamlit so the
    // host UI's region list can follow the user's click on the canvas.
    // Clicks on empty space (`selection:cleared`) intentionally do nothing —
    // the host list always shows some selection.
    const onSelection = () => {
      const obj = canvas.getActiveObject() as any
      const name =
        obj && typeof obj.wos_region_name === "string"
          ? obj.wos_region_name
          : ""
      if (name) {
