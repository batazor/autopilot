...
const sendDataToStreamlit = (
  ...
  Streamlit.setComponentValue({
    ...
    raw: canvas.toObject(),
    // wos-fork: surface the user's last canvas-side selection so the host
    // app can mirror it into its own region list. Empty string when unknown.
    active_region_name: activeRegionName || "",
  })
}
  ...
  useEffect(() => {
    if (debouncedStateToSend && props.shouldSendToStreamlit) {
      stCanvas.loadFromJSON(debouncedStateToSend, () => {
        sendDataToStreamlit(stCanvas, props.activeRegionName)
      })
    }
  }, [
    stCanvas,
    props.shouldSendToStreamlit,
