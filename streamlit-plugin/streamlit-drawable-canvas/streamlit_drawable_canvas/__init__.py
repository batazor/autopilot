...
class CanvasResult:
    ...
    active_region_name: str
        Name (``wos_region_name``) of the rectangle the user last selected on
        the canvas; empty string when no tagged region is active. Used by the
        host app to mirror canvas-side selection into its own region picker.
        (wos-fork extension.)
    """
...
def st_canvas(
    ...
    return CanvasResult(
        ...
        component_value["raw"],
        str(component_value.get("active_region_name") or ""),
    )
