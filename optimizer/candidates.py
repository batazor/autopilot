...
def hero_db_entry(hero_id: str) -> dict[str, Any]:
    """``db/heroes/<hero_id>.yaml`` parsed — cached so candidate gen and
    scorer can share without re-reading disk."""
    if not hero_id:
        return {}
    path = Path(__file__).resolve().parents[1] / "db" / "heroes" / f"{hero_id}.yaml"
    if not path.is_file():
        return {}
