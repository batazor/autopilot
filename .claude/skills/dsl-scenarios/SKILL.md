...
- `state: dotted.path` → the long-lived **SQLite player-state DB** (plain SQLite — *not* a YAML file). Survives restarts; for durable "facts about the player" and drives arithmetic `cond` via the flat state dict.
...
- **CLAUDE.md** (repo root) — authoritative for structure: module layout, the Labeling-UI rule, the SQLite state DB, the Redis key layout, and the `uv` convention.
