...
2. Falls back to scanning template filenames; matches the placeholder against
   the key, validates the captured value against ``db/heroes/index.yaml``,
...
# Hero ids in ``db/heroes/index.yaml`` are lowercase ASCII + underscores.
...
def _hero_index(repo_root_s: str) -> dict[str, str]:
    """``hero_id → display name`` from ``db/heroes/index.yaml``."""
    idx_path = Path(repo_root_s) / "db" / "heroes" / "index.yaml"
    try:
...
def _axis_context(repo_root: Path, axis: str, value: str) -> dict[str, str] | None:
    ...
    hero id that isn't in ``db/heroes/index.yaml``) so the caller can move on
...
def resolve(repo_root: Path, scenario_key: str) -> ResolvedScenario | None:
    """Literal-then-template resolution. ``None`` if neither matches."""
    ...
    if literal_hits:
        literal_hits.sort(
            key=lambda p: (len(p.relative_to(scenarios_root).parts), p.as_posix())
        )
        return ResolvedScenario(path=literal_hits[0], context={})

    for tmpl in _iter_template_yaml_paths(scenarios_root):
...
def iter_resolved_keys(repo_root: Path) -> list[ResolvedKey]:
    """Every concrete scenario key the worker can run.
