"""Lookup helpers shared by the Streamlit pages — kept Streamlit-free so they
can be unit-tested without booting the embedded bot at import time.

``ui/views/instance.py`` imports ``ensure_embedded_bot()`` and registers
``@st.fragment`` handlers at module load. Putting these helpers there
makes tests that just want to call them hang on streamlit setup.
"""
...
@lru_cache(maxsize=1)
def runnable_scenario_keys(scenarios_root_s: str) -> tuple[str, ...]:
    ...
    """
    root = Path(scenarios_root_s)
    out: set[str] = set()
    for p in iter_scenarios_yaml_paths(root):
        if "{" in p.name:
            continue
