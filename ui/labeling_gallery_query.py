...
def open_in_labeling_query_params(labeling_ref: str, card_ver: str) -> dict[str, str]:
    """``st.page_link(..., query_params=…)`` for the labeling deep-link.

    ``labeling_ref`` is the path under ``references/`` (default ``ocr`` for the screen).
    ``card_ver`` is the gallery Layout key: ``auto``, ``default``, or a version id (e.g. ``v2``).
    """
    qp: dict[str, str] = {"ref": labeling_ref}
