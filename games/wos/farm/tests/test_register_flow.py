...
class _FakePage:
    ...
    def emit_register_response(
        ...
        *,
        ...
        response = _FakeResponse(register.REGISTER_API_URL, status, body, method)
...
class _ClosedOnGotoPage(_FakePage):
    async def goto(self, url: str, **_kw: object) -> None:
        self.calls.append(("goto", url))
...
class _FakeBrowser:
    ...
    async def new_context(self, **_kw: object) -> _FakeContext:
...
class _FakeChromium:
    ...
    async def launch(self, **_kw: object) -> _FakeBrowser:
...
async def test_drive_fills_then_hands_off_then_detects_success() -> None:
    ...
    async def on_ready(automation: register.AutomationReport) -> None:
        ...
        page.emit_register_response(200)
...
async def test_drive_reports_failure_when_register_response_never_arrives() -> None:
    page = _FakePage()

    async def on_ready(_automation: register.AutomationReport) -> None:
        pass  # operator did nothing and sdk/register never fired

    ok = await register.drive_registration(
        page,
        _acct(),
        on_ready,
        register_response_timeout_ms=1,
    )
    assert ok is False


async def test_register_response_success_does_not_need_done() -> None:
    # The operator can click Sign Up in the browser and never press dashboard Done.
    # The SDK register API HTTP 200 + body code 200 is the final success signal.
    page = _FakePage()

    async def click_sign_up(_automation: register.AutomationReport) -> None:
        page.emit_register_response(200)
        await _never()

    assert await register.drive_registration(page, _acct(), click_sign_up) is True
    assert page.visible is True  # modal heuristic would have said False


async def test_failed_outcome_does_not_wait_for_register_response() -> None:
    page = _FakePage()
...
async def test_non_200_register_response_fails() -> None:
    page = _FakePage()

    async def click_sign_up(_automation: register.AutomationReport) -> None:
        page.emit_register_response(400)

    assert (
        await register.drive_registration(
            page,
            _acct(),
            click_sign_up,
            register_response_timeout_ms=1,
...
async def test_http_200_but_non_200_body_code_fails() -> None:
    page = _FakePage()

    async def click_sign_up(_automation: register.AutomationReport) -> None:
        page.emit_register_response(
            200,
            {
                "code": 400,
    ...
    assert await register.drive_registration(page, _acct(), click_sign_up) is False


async def test_validation_failure_retries_same_account() -> None:
    page = _FakePage()
    events: list[register.AutomationReport] = []

    async def said_done(automation: register.AutomationReport) -> bool:
        events.append(automation)
        if len(events) == 1:
            page.emit_register_response(
                400,
                {"message": "验证失败，请重试", "code": 1024},
            )
        else:
            page.emit_register_response(200)
        return True
    ...
    assert [e["register_attempt"] for e in events] == ["1", "2"]
    assert "body_code=1024" in events[1]["previous_register"]


async def test_validation_failure_stops_after_retry_limit() -> None:
    page = _FakePage()
    attempts = 0
...
def test_register_account_marks_closed_browser_failed(monkeypatch) -> None:
    page = _ClosedOnGotoPage()
    fake = _FakePlaywright(page)
    monkeypatch.setattr(register, "_require_playwright", lambda: lambda: fake)

    async def done(_automation: register.AutomationReport) -> None:
        msg = "human handoff should not run when navigation closes"
        raise AssertionError(msg)

    ok = register.register_account(_acct(), done=done)

    assert ok is False
    assert fake.chromium.browser is not None
