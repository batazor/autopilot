#!/usr/bin/env python3
"""Import Dreamscape Memory **Season 3** maps (Shield WOS sheet) into the scene DB.

Like the Season 2 sheet, this sheet embeds a *numbered guide image* per map. We
pull the xlsx export, extract the left-column guide image for each map (validated
1:1 by row order), parse the item-name list, and upsert each as a ``<slug>-s3``
scene — the suffix avoids clobbering Season 1/2 scenes that share a room name
(garden/museum/aquarium/garage/…).

Images land in ``references/maps/<slug>-s3.png``. The sheet has no coordinates,
...
(slow) OCR pass and keep every pin on the grid.
    ...
    uv run python games/wos/events/dreamscape_memory/tools/import_maps_s3.py [--dry-run] [--no-ocr]
"""
...
SHEET_ID = "1EKn5-72QeqRzh_Wmii8Nb86L5US7VllJviMnYPPPUJQ"
    ...
    for (title, items), img in zip(maps, images):
        ...
        if not dry:
            ...
            dreamscape_db.upsert_scene(
                ...
                points=points,
                activate=False,
                archived=False,  # Season 3 is current content
                season=3,
            )
