...
@pytest.mark.asyncio
async def test_deals_detect_tabs_holds_when_only_active_tab_is_dotted(
    area_doc: dict,
) -> None:
    """Regression: endless_wayfarer's milestone red dot sits on the *active* tab
    and its claim scenario can't clear it. The detectTabs rule must NOT push the
    tab-advance helper for an active-only red dot — the navigator would only
    ``hold`` and the rule would re-fire every tick (infinite loop that starved
    the whole scheduler). With no inactive tab to switch to, push nothing."""
    frame = _load_bgr(
        "games/wos/deals/endless_wayfarer/references/endless_wayfarer.active_tab_red_dot.png"
    )
    rules = [
        {
            "name": "deals.tabs.visible_red_dot",
            ...
            "namespace": "deals",
            "push_red_dot_pages": True,
            "screens": ["deals.endless_wayfarer"],
            "steps": [
                {
                    "push_scenario": {
                        "name": "tabs.strip.advance",
    ...
    hit = out["deals.tabs.visible_red_dot"]
    # The only red dot is on the active tab.
    assert hit["active_index"] is not None
    assert hit["red_dot_indices"] == [hit["active_index"]]
    assert hit["red_dot_pages"] == []
    assert hit["tab_action"] == "active_red_dot_hold"
    assert "pushScenario" not in hit
...
def test_deals_hub_icon_has_recheck_cooldown() -> None:
    """The main_city deals icon re-opens the hub at most every 30m. A short ttl
    re-opened it every minute because purchase-only deals keep its red dot lit
    forever (main_city <-> deals churn); the in-panel tab-advance helper must
    stay responsive at 15s."""
    from analysis.overlay_manifest import load_analyze_yaml
    ...
    icon = next(
        ...
        if r.get("name") == "deals.main_city.event_icon.visible"
    )
    (push,) = [s["push_scenario"] for s in icon["steps"] if "push_scenario" in s]
    assert push["name"] == "deals.deals"
    assert str(push["ttl"]) == "30m"

    for rule in doc["overlay"]:
        for step in rule.get("steps") or []:
            ps = step.get("push_scenario") if isinstance(step, dict) else None
            if ps and ps.get("name") == "tabs.strip.advance":
                assert str(ps["ttl"]) == "15s"
