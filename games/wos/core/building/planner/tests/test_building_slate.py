...
def _graph():
    ...
    storehouse = BuildingSpec("storehouse", "Storehouse", (
        _lvl("1", 1.0, (("furnace", 1.0),), (("w", 6),)),
    ))
    return BuildGraph(
        buildings={b.id: b for b in (furnace, sawmill, shelter, camp, storehouse)}
    )
...
def test_storehouse_built_by_mains_excluded_by_farm():
    # A main develops the Storehouse (protected-resource cap) as a low-value
    # economy fill; a farm opts out entirely so its pile stays plunderable.
    levels = {"furnace": 1}
    main = plan_builds(G, levels, free_queues=2, resources=RICH, role=get_role("balanced"))
    assert any(c.spec_id == "storehouse" for c in main.candidates)
    farm = plan_builds(G, levels, free_queues=2, resources=RICH, role=get_role("farm"))
    assert all(c.spec_id != "storehouse" for c in farm.candidates)
    assert all(p.spec_id != "storehouse" for p in farm.picks)


def test_all_maxed():
    levels = {"furnace": 2, "sawmill": 2, "infantry_camp": 1, "storehouse": 1}
