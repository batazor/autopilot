...
def plan_builds(
    ...
    *,
    ...
    pick competes with *economy* (resource producers + the 8 Shelters) and *camp*
    candidates by value; the top affordable+ready ones fill the free queues. So a
    free queue never idles — when the furnace pick is unaffordable, the queue builds
    a producer / Shelter instead. ``role`` tilts economy↔battle; progression stays
    universal. Bottleneck repair (short resource → its producer) boosts that
    producer's value, but is inert until ``policy.PRODUCER_BY_ITEM`` is filled.
    """
    ...
    for spec_id in (*policy.PRODUCERS, policy.SHELTER_ID):
        if graph.spec(spec_id) is None:
            continue
        kind = "shelter" if spec_id == policy.SHELTER_ID else "producer"
        value = policy.building_value(kind, role)
