"""Static hero catalog parsed from ``games/wos/core/heroes/wiki/heroes/*.yaml``.
...
# games/wos/core/heroes/planner/ → parents[1] = heroes
DEFAULT_HEROES_DIR = Path(__file__).resolve().parents[1] / "wiki" / "heroes"
...
def load_hero_catalog(directory: str | Path | None = None) -> dict[str, HeroSpec]:
    ...
    d = Path(directory) if directory else DEFAULT_HEROES_DIR
