...
def test_march_config_defaults_off_when_unset(tmp_path, monkeypatch):
    # Parser default: a config with no `enabled` key is OFF (fail-safe).
    monkeypatch.delenv("WOS_MARCH_ENABLED", raising=False)
    p = tmp_path / "march.yaml"
    p.write_text("intel_cooldown_s: 900\n", encoding="utf-8")
    cfg = load_march_config(p)
    assert cfg.enabled is False
    assert cfg.intel_cooldown_s == 900


def test_march_config_live_yaml_is_enabled(monkeypatch):
    # The shipped march.yaml is flipped on.
    monkeypatch.delenv("WOS_MARCH_ENABLED", raising=False)
    assert load_march_config().enabled is True


def test_march_env_killswitch_overrides_yaml(monkeypatch):
    monkeypatch.setenv("WOS_MARCH_ENABLED", "false")
    assert load_march_config().enabled is False      # kill-switch beats the live yaml
    monkeypatch.setenv("WOS_MARCH_ENABLED", "true")
    assert load_march_config().enabled is True
