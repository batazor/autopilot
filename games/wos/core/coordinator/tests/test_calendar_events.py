...
def test_icefire_warhymn_league_boosts_gather_and_combat():
    # Non-phased mission-track event: gather + hunt Polar Terrors score points.
    bias = calendar_bias([EventWindow("icefire_warhymn_league", active=True, ends_in_s=DAY)])
    for d in ("gather", "building_economy", "raids"):   # GATHER + COMBAT category domains
        assert bias.domain_boost[d] == DEFAULT_BOOST
    assert "research" not in bias.domain_boost          # research doesn't score league points
    assert bias.needs_read == ()                        # reward mix known → nothing to read live
    assert bias.holds and bias.holds[0].until_s == 0.0  # speedups score → "spend now" hold
