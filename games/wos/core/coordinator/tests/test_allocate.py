...
def test_optimal_beats_greedy_when_the_budget_binds():
    ...
    a = CandidateAction("d", CONSTRUCTION, "A", 1000.0, {"wood": 100})
    b = CandidateAction("d", CONSTRUCTION, "B", 600.0, {"wood": 50})
    c = CandidateAction("d", CONSTRUCTION, "C", 600.0, {"wood": 50})
    bal = {"wood": 100}
...
def test_matches_greedy_when_nothing_contends():
    ...
    cands = [
        CandidateAction("building_progression", CONSTRUCTION, "furnace", 850.0, {}),
        CandidateAction("research", "research", "tech", 900.0, {"wood": 10}),
    ]
    bal = {"wood": 1000}
...
def test_march_stamina_knapsack_picks_the_best_affordable_set():
    ...
    hi = CandidateAction("intel", MARCH, "hi", 760.0, {"stamina": 10})
    mid = CandidateAction("intel", MARCH, "mid", 700.0, {"stamina": 10})
    lo = CandidateAction("intel", MARCH, "lo", 600.0, {"stamina": 10})
...
def test_multi_resource_budget():
    chans = [Channel("c1", CONSTRUCTION), Channel("c2", CONSTRUCTION)]
    a = CandidateAction("d", CONSTRUCTION, "A", 500.0, {"wood": 80, "iron": 10})
    b = CandidateAction("d", CONSTRUCTION, "B", 500.0, {"wood": 80, "iron": 10})
...
def test_candidates_without_a_channel_are_reported():
    ...
    on = CandidateAction("building_progression", CONSTRUCTION, "furnace", 850.0, {})
    off = CandidateAction("heroes", "hero", "natalia", 580.0, {})
...
def test_falls_back_to_greedy_on_node_overflow():
    ...
    chans = [Channel("c1", CONSTRUCTION), Channel("c2", CONSTRUCTION)]
    cands = [
        CandidateAction("d", CONSTRUCTION, k, p, {"wood": 50})
