"""``exec: sync_gear_owned`` — OCR'd gear pieces → planner['gear']['owned']."""
...
def test_handler_is_registered() -> None:
    assert "sync_gear_owned" in build_dsl_exec_registry()


@pytest.mark.asyncio
async def test_persists_gear_pieces() -> None:
    redis = _FakeRedis({PKEY: {
        "gear.read.gloves_belt_infantry": "5",
        "gear.read.goggles_boots_marksman": "3",
    }})
    ctx = _ctx(redis)

    await build_dsl_exec_registry()["sync_gear_owned"](ctx)

    assert ctx.result == {"action": "synced", "pieces": 2, "player_id": PLAYER}
    owned = json.loads(redis.store[PKEY]["gear.owned"])
    assert owned == {"gloves_belt_infantry": 5, "goggles_boots_marksman": 3}
    assert json.loads(redis.store[IKEY]["gear.owned"]) == owned
...
async def test_noop_when_no_pieces_read() -> None:
    ...
    assert ctx.result == {}
    assert "gear.owned" not in redis.store.get(PKEY, {})
