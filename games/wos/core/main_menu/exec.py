...
async def _exec_sync_main_menu_research_status(ctx: DslExecContext) -> None:
    ...
    """
    if ctx.redis_client is None:
        logger.warning("dsl exec sync_main_menu_research_status: no redis client")
        ctx.result.update({"reason": "no_redis_client"})
        return

    player_id = await _resolve_player_id_for_device_level_exec(ctx)
    if not player_id:
        logger.warning("dsl exec sync_main_menu_research_status: empty player_id")
        ctx.result.update({"reason": "empty_player_id"})
        return
...
async def _exec_tap_main_menu_panel_row(ctx: DslExecContext) -> None:
    ...
    for _ in range(4):
        ...
        if not ok:
            ctx.result.update({"reason": "swipe_not_approved"})
            return
        await asyncio.sleep(0.5)

    for sweep in range(_TAP_PANEL_ROW_MAX_SWEEPS):
        try:
            ...
            logger.exception(
                "dsl exec tap_main_menu_panel_row: capture failed instance=%s",
                ctx.instance_id,
            )
            ctx.result.update({"reason": "capture_failed"})
            return
        ...
        target = next(
            (
                r
                for r in rows
                if r["section"] == section and r["row"] == row and r["button"]
            ),
            None,
        )
        if target is not None:
            ...
            tapped = bool(
                await asyncio.to_thread(
                    ...
                    or f"main_menu.panel.{section}.{row}",
                )
            )
            ctx.result.update(
                {
    ...
    ctx.result.update({"reason": "row_not_found", "section": section, "row": row}
    )
...
async def _exec_tap_training_accept(ctx: DslExecContext) -> None:
    ...
    # Return to the top of the panel first — the row hunt below only scrolls
    # forward (content up), so start from a known scroll origin.
    for _ in range(4):
        ok = await asyncio.to_thread(
            actions.swipe_direction,
            ctx.instance_id,
    ...
    for sweep in range(_TAP_ACCEPT_MAX_SWEEPS):
        try:
            image = await asyncio.to_thread(
                actions.capture_screen_bgr, ctx.instance_id
            )
            ...
            logger.exception(
                "dsl exec tap_training_accept: capture failed instance=%s",
                ctx.instance_id,
            )
            ctx.result.update({"reason": "capture_failed"})
            return
        ...
        target = next(
            (
                r
                for r in rows
                if r["section"] == "training" and r["row"] == troop and r["button"]
            ),
            None,
        )
        if target is not None:
    ...
    ctx.result.update({"reason": "row_not_found", "troop": troop})
